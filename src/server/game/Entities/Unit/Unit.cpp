...
bool Unit::IsImmunedToMechanic(uint32 mechanic) const
{
    SpellImmuneContainer const& mechanicList = m_spellImmune[IMMUNITY_MECHANIC];
    for (auto itr = mechanicList.begin(); itr != mechanicList.end(); ++itr)
        if ((itr->first & mechanic) != 0) // (need to verify !!! logic imported from Colossus without test)
            return true;

    return false;
}

bool Unit::IsPermanentlyImmunedToMechanic(uint32 mechanic) const
{
    bool permanent = false;
    SpellImmuneContainer const& mechanicList = m_spellImmune[IMMUNITY_MECHANIC];
    for (auto itr = mechanicList.begin(); itr != mechanicList.end(); ++itr)
        if ((itr->first & mechanic) != 0) // (need to verify !!! logic imported from Colossus without test)
            if (Aura* immuneAura = GetAura(itr->second))
                if (immuneAura->IsPermanent())
                    return true;
...
void Unit::GetFriendlyUnitListInRange(const WorldObject* obj, std::list<Unit*> &list, float fMaxSearchRange) const
{
    CellCoord p(Trinity::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    Trinity::AnyFriendlyUnitInObjectRangeCheck u_check(obj, this, fMaxSearchRange);
...
void Unit::GetAttackableUnitListInRange(const WorldObject* obj, std::list<Unit*> &list, float fMaxSearchRange) const
{
    CellCoord p(Trinity::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    Trinity::AnyUnfriendlyNoTotemUnitInObjectRangeCheck u_check(obj, this, fMaxSearchRange);
    Trinity::UnitListSearcher<Trinity::AnyUnfriendlyNoTotemUnitInObjectRangeCheck> searcher(this, list, u_check);

    TypeContainerVisitor<Trinity::UnitListSearcher<Trinity::AnyUnfriendlyNoTotemUnitInObjectRangeCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
    TypeContainerVisitor<Trinity::UnitListSearcher<Trinity::AnyUnfriendlyNoTotemUnitInObjectRangeCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
void Unit::NearTeleportTo(uint32 worldSafeLocId, bool casting)
{
...
void Unit::CastExpelHarmDamage(uint32 damage)
{
    if (Player* _player = this->ToPlayer())
    {
        ...
        _player->GetAttackableUnitListInRange(_player, targetList, radius);
