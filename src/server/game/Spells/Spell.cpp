...
void Spell::SelectImplicitChainTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, WorldObject* target, uint32 effMask)
{
    ...
    // Spell Havoc, Warlock family
    if (m_caster->HasSpell(80240) && target->ToUnit() && !target->ToUnit()->HasAura(80240))
        if ((GetSpellInfo()->HasEffect(SPELL_EFFECT_SCHOOL_DAMAGE) || GetSpellInfo()->HasEffect(SPELL_EFFECT_HEALTH_LEECH) || GetSpellInfo()->HasAura(DIFFICULTY_NONE, SPELL_AURA_PERIODIC_LEECH)) && (!GetSpellInfo()->IsTargetingArea(DIFFICULTY_NONE) || (GetSpellInfo()->Id == 29722 && !m_caster->HasAura(196408))))
            if (Aura const* destroWarlock = m_caster->GetAura(137046))
                if (ObjectGuid const* havocTargetGUID = destroWarlock->GetCastExtraParam<ObjectGuid>("HavocTargetGUID"))
                    if (Unit* havocTarget = ObjectAccessor::GetUnit(*m_caster, *havocTargetGUID))
