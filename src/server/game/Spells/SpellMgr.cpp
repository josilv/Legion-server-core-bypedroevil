...
void SpellMgr::LoadSpellInfoCorrections()
{
    ...
    // 209426 Darkness buff
    ApplySpellFix({ 209426 }, [](SpellInfo* spellInfo)
    {
        spellInfo->AttributesEx5 |= SPELL_ATTR5_HIDE_DURATION;
    });

    // 226778 Infernal Strike
    ApplySpellFix({ 226778 }, [](SpellInfo* spellInfo)
    {
        ...
        spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(584);
    ...
    ApplySpellFix({ 202425 }, [](SpellInfo* spellInfo)
    {
        spellInfo->ProcFlags = 0;
        spellInfo->ProcBasePPM = 0;
        spellInfo->ProcChance = 0;
        spellInfo->ProcCharges = 0;
        spellInfo->ProcCooldown = 0;
    });

    // 197637 Potenciación estelar
    ApplySpellFix({ 197637 }, [](SpellInfo* spellInfo)
    {
        const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN;
        const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->SpellClassMask = flag128(0x40002, 0, 0, 0);
    });

    // 115315 Invocar estatua del Buey Negro
    ApplySpellFix({ 115315 }, [](SpellInfo* spellInfo)
    {
        const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->Effect = SPELL_EFFECT_APPLY_AURA;
        const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->ApplyAuraName = SPELL_AURA_DUMMY;
    });

    // Producto de la duda,  Robar alma
    ApplySpellFix({ 106935, 136757 }, [](SpellInfo* spellInfo)
    {
        const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->Effect = SPELL_EFFECT_SCRIPT_EFFECT;
        const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->ApplyAuraName = 0;
    });
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i)
    {
        ...
        switch (spellInfo->Id)
        {
        case 195181: //195181 Bone Shield
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_4))->ApplyAuraName = SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN;
            break;
        case 215806: // Tail Lash
            spellInfo->AttributesCu |= SPELL_ATTR0_CU_CONE_BACK;
            break;
        case 203650: //Prepared - Demon Hunter
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_PERIODIC_ENERGIZE;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraPeriod = 1000;
            break;
        case 194249: //Voidform Shadow Priest hack - This prevent client wow error from this spell, we need try to find a better way to solve this wow error
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_12))->Effect = SPELL_EFFECT_DUMMY;
            break;
        case 196834: // Frostbrand
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->ApplyAuraName = SPELL_AURA_PROC_TRIGGER_SPELL;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->TriggerSpell = 210854;
            break;
        case 197568: // Lightning Rod
            spellInfo->ProcFlags = 0;
            spellInfo->ProcBasePPM = 0;
            spellInfo->ProcChance = 0;
            spellInfo->ProcCharges = 0;
            spellInfo->ProcCooldown = 0;
            break;
        case 114052: // Ascendance (Water)
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->ApplyAuraName = SPELL_AURA_DUMMY;
            break;
        case 115357: // Windstrike (Ascendance 114051)
        case 115360: // Windstrike Off-Hand (Ascendance 114051)
        case 114089: // Windlash (Ascendance 114051)
            spellInfo->AttributesCu |= SPELL_ATTR0_CU_IGNORE_ARMOR;
            break;
        case 198103: //Earth Elemental
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_DUMMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = 0;
            spellInfo->DurationEntry = 0;
            break;
        case 202192: //Resonance Totem 
            spellInfo->CastTimeEntry = 0;
            break;
        case 170374: // Earthen Rage
            spellInfo->AttributesEx8 = 0;
            break;
        case 200153: // Enduring Renewal
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_TARGET_ALLY;
            spellInfo->ProcChance = 100;
            spellInfo->ProcCharges = 0;
            spellInfo->ProcFlags = 0x00044000;
            break;
        case 197915: // Lifecycles
            spellInfo->ProcCharges = 0;
            spellInfo->StackAmount = 0;
            break;
        case 124506: // Gift of the Ox Healing Sphere Left
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_DEST_CASTER_LEFT;
            break;
        case 124502: // Gift of the Ox
            ...
            break;
        case 235711: // Chrono Shift trigger
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            spellInfo->RangeIndex = 5;
            spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(5);
            break;
        case 195283: // Hot Streak
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_DUMMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_CASTER;
            spellInfo->ProcChance = 100;
            spellInfo->ProcCharges = 0;
            spellInfo->ProcFlags = 0x00010000;
            break;
        case 153596:// Comet Storm
        case 228601:// Comet Storm Visual
            spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
        ...
        case 108370:
            ...
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->Effect = SPELL_EFFECT_APPLY_AURA;
            break;
        case 119996: //Transcendence: Transfer
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_CASTER;
            break;
        case 204022:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_MOD_DAMAGE_PERCENT_DONE;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->BasePoints = -40;
            break;
        ...
        case 126084:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->SpellClassMask = flag128(0x20000, 0, 0, 0x100);
        ...
        case 193196:
            ...
            // ColossusWoW Fixes
            //
            // This spells will cause error when press ESC button this will fix it.
        case 67499: // The Great Bank Heist: Bunny - G-Ray Aura
        case 67526: // Amazing G-Ray
        case 67496: // The Great Bank Heist: Bunny - Explosives Aura
        case 67508: // Blastcrackers
        case 67497: // The Great Bank Heist: Bunny - Listen Aura
        case 67524: // Ear-O-Scope
        case 67498: // The Great Bank Heist: Bunny - Lockpick Aura
        case 67525: // Infinifold Lockpick
        case 67495: // The Great Bank Heist: Bunny - Drill Aura
        case 67522: // Kaja'mite Drill
        case 67493: // The Great Bank Heist: Power Correct
        case 67494: // The Great Bank Heist: Power Incorrect
            spellInfo->AttributesEx4 &= ~SPELL_ATTR4_CAN_CAST_WHILE_CASTING;
            break;
        case 66393: // Rolling with my Homies: Summon Hot Rod
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_SUMMON;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->BasePoints = 66392;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->MiscValue = 34840;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->MiscValueB = 827;
            spellInfo->AttributesEx4 &= ~SPELL_ATTR4_CAN_CAST_WHILE_CASTING;
            break;
        case 102207: // Invocar aire viviente
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_CASTER;
            break;
        case 76577: // Smoke Bomb
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_DEST_CASTER;
            break;
        case 139341: // Smoke bomb
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = 0;
            break;
        case 131347: // Glide
            spellInfo->AuraInterruptFlags[0] |= AURA_INTERRUPT_FLAG_LANDING;
        ...
        case 5255:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetB = SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
            break;
        case 189831:
            spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(24);
            break;
        case 221461:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_PERIODIC_DUMMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraPeriod = 1000;
        ...
        case 109248: // Binding shot
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_DEST_DEST;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetB = TARGET_UNIT_DEST_AREA_ENEMY;
            break;
        case 203526: // Neltharion's Fury
            spellInfo->AttributesEx &= ~SPELL_ATTR0_IMPOSSIBLE_DODGE_PARRY_BLOCK;
            break;
        case 31935: // Avengers Shield
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_SCHOOL_DAMAGE;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ChainTargets = 3;
            break;
        case 8188: // Magma Totem Passive
        case 5672: // Healing Streams
        case 114941: // Healing Tide
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_PERIODIC_TRIGGER_SPELL;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Amplitude = 2000;
            break;
        case 185763: // Pistol Shot
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_SCHOOL_DAMAGE;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_MOD_DECREASE_SPEED;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            break;
        case 204883: // Circle of Healing
        case 596: // Skartax Purple Beam
            spellInfo->MaxAffectedTargets = 5;
            break;
        case 191284:
            break;
        case 203955:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = SpellImplicitTargetInfo(TARGET_DEST_DEST_RADIUS);
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetB = SpellImplicitTargetInfo(TARGET_DEST_DEST_RADIUS);
            break;
        ...
        case 152150://< Death from Above (periodic dummy)
            ...
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_5))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_5))->ApplyAuraName = SPELL_AURA_PERIODIC_DUMMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_5))->ApplyAuraPeriod = 1000;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_5))->TargetA = TARGET_UNIT_CASTER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_5))->TriggerSpell = 156327;
            spellInfo->PreventionType = SPELL_PREVENTION_TYPE_PACIFY;
            break;
        case 178236://< Death from Above (jump dest)
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_DEST_TARGET_BACK;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetB = TARGET_DEST_TARGET_BACK;
            break;
        case 178153: ///< Death from Above
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetA = TARGET_DEST_TARGET_FRONT;
            break;
        case 156327: //< Death from Above (jump to target)
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetB = 0;
            break;
        case 1463: // Incanter's Ward
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraPeriod = 2000;
            break;
        case 137639: // Storm, Earth and Fire
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_DUMMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->ApplyAuraName = SPELL_AURA_DUMMY;
            break;
        ...
        case 115072: // Expel Harm
            ...
            break;
        case 115129:// Expel Harm - Damage to a nearby enemy within 10 yards
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_SRC_CASTER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetB = TARGET_UNIT_NEARBY_ENEMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->RadiusEntry = sSpellRadiusStore.LookupEntry(13);
            break;
        case 53209: // Chimera Shot
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ChainTargets = 2;
            break;
        case 116740:// Tigereye Brew
            spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(1);
            break;
        case 199483: // Camuflaje
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_PET;
            break;
        case 774: // Rejuvenation
            spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(29); // 12 secs
            break;
        case 192477: // Teleport: The Dark Lady's Fleet
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->MiscValue = 96686;
            break;
        case 192465: // Teleport: The Skyfire
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->MiscValue = 96663;
            break;
        case 214871: // Odyn's Fury not immortal now.
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = 0;
            break;
        case 198670:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->BasePoints = 500;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetB = 0;
            ...
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_4))->Effect = 0;
            break;
        case 62371: // Spirit of redemption immunity
            sSpellDurationStore.LookupEntry(8); // 15 secs
            break;
        case 20711: // Spirit of redemption
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->BasePoints = 0;
            break;
        case 214482: // Radiant Ley Crystal
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            break;
        case 223473: // Shear
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = SpellImplicitTargetInfo(TARGET_DEST_TARGET_ENEMY);
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetB = 0;
            break;
        case 79140: // Vendetta
            spellInfo->AttributesEx |= SPELL_ATTR1_NOT_BREAK_STEALTH;
            break;
        case 231390: // Trailblazer
            spellInfo->AuraInterruptFlags[0] = AURA_INTERRUPT_FLAG_HITBYSPELL | AURA_INTERRUPT_FLAG_TAKE_DAMAGE;
            break;
        case 196353: // Glide knockback
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_LEAP_BACK;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->BasePoints = 75;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->MiscValue = 125;
            break;
        case 206930: // Heart Strike
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->TargetA = TARGET_UNIT_CASTER;
            break;
        case 202770: // Fury of Elune
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->ApplyAuraName = SPELL_AURA_PERIODIC_DUMMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->ApplyAuraPeriod = 1000;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->TargetA = TARGET_UNIT_CASTER;
            break;
        case 205320:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->TargetB = TARGET_DEST_DEST_FRONT;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_3))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_3))->TargetB = TARGET_DEST_DEST_FRONT;
            break;
        case 222029: // Golpe del señor del viento
        case 205414:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = SpellImplicitTargetInfo(TARGET_DEST_TARGET_ENEMY);
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetB = TARGET_DEST_DEST_FRONT;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetA = SpellImplicitTargetInfo(TARGET_DEST_TARGET_ENEMY);
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetB = TARGET_DEST_DEST_FRONT;
            break;
        case 228598:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            break;
        case 201427: // Annihilation
            ...
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            break;
        case 193743: // Aegis of Aggramar absorb
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->BasePoints = 999999999;
            break;
        case 198072:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            break;
        ...
        case 200682:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_SRC_CASTER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetB = TARGET_DEST_TARGET_ANY;
            break;
        case 227539:
        case 227566:
        case 227570:
        ...
        case 232807:
            ...
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetB = 0;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->TargetA = TARGET_UNIT_TARGET_ENEMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_2))->TargetB = 0;
            spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(3);
            break;
        case 228027: // Fury of the Maw
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_AREA_TRIGGER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->MiscValue = 8829;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_CASTER;
            break;
        case 228058: // Orb of Corrosion
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetA = TARGET_UNIT_CASTER;
            break;
        case 201706:
            spellInfo->ProcChance = 100;
            break;
        case 227782: // Glowing Fragment
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_AREA_TRIGGER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->MiscValue = 8810;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_CASTER;
            break;
        case 227455: // Cleasing Flame
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_DEST_DEST;
            break;
        ...
        case 84714:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_AREA_TRIGGER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->MiscValue = 8661;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_CASTER;
            break;
        case 110744:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_AREA_TRIGGER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->MiscValue = 2148;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_UNIT_CASTER;
            break;
        case 22568:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->Effect = SPELL_EFFECT_ENERGIZE;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->BasePoints = -25;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->MiscValue = 3;
            break;
        case 194594:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->BasePoints = -2000;
            break;
        case 212431:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->ApplyAuraName = SPELL_AURA_AREA_TRIGGER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->MiscValue = 7019;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetA = TARGET_UNIT_TARGET_ANY;
            break;
        case 123986:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Effect = SPELL_EFFECT_DUMMY;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = TARGET_DEST_CASTER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->ApplyAuraName = SPELL_AURA_AREA_TRIGGER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->MiscValue = 1315;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetA = TARGET_UNIT_TARGET_ANY;
            spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(21);
            break;
        case 230345:
        ...
        case 233279:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->Effect = SPELL_EFFECT_APPLY_AURA;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->ApplyAuraName = SPELL_AURA_AREA_TRIGGER;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->MiscValue = 9417;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->TargetA = TARGET_UNIT_TARGET_ANY;
            break;
        case 242984:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraName = SPELL_AURA_PERIODIC_TRIGGER_SPELL;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->ApplyAuraPeriod = 1000;
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TriggerSpell = 242906;
            break;
        case 218104:
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->TargetA = SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ENEMY);
            break;
