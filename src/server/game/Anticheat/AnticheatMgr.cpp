/*
* Copyright (C) 2018 ShadowMelt Core.
*/
...
void AnticheatMgr::BuildReport(Player* player, uint8 reportType)
{
    ...
    if (player->anticheatData.total_reports >= sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION))
    {
        ...
        std::string str = "| ShadowMelt AntiCheats|cFF00FFFF[|cFF60FF00" + std::string(player->GetName()) + "|cFF00FFFF] Posible cheater! Tipo: "+ report_type;
        ...
        /*if (autojail > 2) // 0,1,2,3 - computers start count from 0
        {
            ...
            ChatHandler(player->GetSession()).PSendSysMessage("You has been jailed by: ColossusWoW AntiCheats 2.0");
