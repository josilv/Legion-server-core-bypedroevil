...
enum DeathKnightSpells
{
    ...
    SPELL_DK_REMORSELESS_WINTER_SLOW_DOWN       = 211793,
        ...
        SPELL_DK_BREATH_OF_SINDRAGOSA_HEAL = 155168,
...
public:
    ...
    class spell_dk_frost_strike_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (GetCaster()->HasAura(Spells::IcyTalons))
                GetCaster()->CastSpell(GetCaster(), Spells::IcyTalonsAurastate, true);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_frost_strike_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };
...
// 207142 Avalanche 
/// 7.1.5
class spell_dk_avalanche : public SpellScriptLoader
{
public:
    spell_dk_avalanche() : SpellScriptLoader("spell_dk_avalanche") { }

    class spell_dk_avalanche_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_dk_avalanche_AuraScript);
        ...
        void HandleProc(ProcEventInfo& eventInfo)
        {
            if (!(eventInfo.GetHitMask() & PROC_HIT_CRITICAL && GetCaster()->HasAura(Spells::PillarOfFrost)))
                PreventDefaultAction();
        }

        void Register() override
        {
            OnProc += AuraProcFn(spell_dk_avalanche_AuraScript::HandleProc);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_dk_avalanche_AuraScript();
    }
};
...
public:
    ...
    class spell_dk_purgatory_AuraScript : public AuraScript
    {
        ...
        void Absorb(AuraEffect* auraEffect, DamageInfo& dmgInfo, uint32& absorbAmount)
        {
            ...
            if (GetCaster()->HasAura(Spells::Perdition))
                return;

            if (dmgInfo.GetDamage() >= GetCaster()->GetHealth())
            {
                absorbAmount = dmgInfo.GetDamage();
                GetCaster()->CastCustomSpell(Spells::ShroudOfPurgatory, SPELLVALUE_BASE_POINT0, absorbAmount, GetCaster(), true);
                GetCaster()->CastSpell(GetCaster(), Spells::Perdition, true);
            }
        }

        void Register()
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dk_purgatory_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
            OnEffectAbsorb += AuraEffectAbsorbFn(spell_dk_purgatory_AuraScript::Absorb, EFFECT_0);
        }
    };
...
/// 7.1.5
class spell_dk_shroud_of_purgatory : public SpellScriptLoader
{
public:
    spell_dk_shroud_of_purgatory() : SpellScriptLoader("spell_dk_shroud_of_purgatory") { }

    class spell_dk_shroud_of_purgatory_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_dk_shroud_of_purgatory_AuraScript);

        void AfterRemove(AuraEffect const* aurEff, AuraEffectHandleModes mode)
        {
            if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE && aurEff->GetAmount() > 0)
                GetCaster()->KillSelf();
        }

        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(spell_dk_shroud_of_purgatory_AuraScript::AfterRemove, EFFECT_0, SPELL_AURA_SCHOOL_HEAL_ABSORB, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_dk_shroud_of_purgatory_AuraScript();
    }
};
...
public:
    ...
    class spell_dk_crimson_scourge_AuraScript : public AuraScript
    {
        ...
        {
            PreventDefaultAction();
            if (procInfo.GetDamageInfo()->GetVictim()->HasAura(Spells::BloodPlague, GetCaster()->GetGUID()))
            {
                GetCaster()->CastSpell(GetCaster(), CrimsonScourgeTriggered, true);
                GetCaster()->ToPlayer()->GetSpellHistory()->ResetCooldown(Spells::DeathAndDecay, true);
            }
        }

        void Register() override
        {
            OnEffectProc += AuraEffectProcFn(spell_dk_crimson_scourge_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
        }
    };
...
// 155166 - Breath of Sindragosa damage
...
public:
    ...
    class spell_dk_breath_of_sindragosa_damage_SpellScript : public SpellScript
    {
        ...
        void HandleDamage(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
            {
                int32 healthPct = CalculatePct(caster->GetMaxHealth(), 4);
                caster->CastCustomSpell(caster, SPELL_DK_BREATH_OF_SINDRAGOSA_HEAL, &healthPct, nullptr, nullptr, true);
            }
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_breath_of_sindragosa_damage_SpellScript::HandleDamage, EFFECT_1, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };
...
//  227869 Six-Feather Fan (Level 110)
class spell_dk_six_feather_fan : public SpellScriptLoader
{
public:
    spell_dk_six_feather_fan() : SpellScriptLoader("spell_dk_six_feather_fan") { }

    class spell_dk_six_feather_fan_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_dk_six_feather_fan_AuraScript);
        ...
        void HandlePeriodicDummy(AuraEffect const* aurEff)
        {
            ...
            {
                GetCaster()->CastSpell(target, 227870, true);
            }
        }

        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_dk_six_feather_fan_AuraScript::HandlePeriodicDummy, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_dk_six_feather_fan_AuraScript();
    }
};

// 227870 Wind Bolt (Level 110)
class spell_dk_six_feather_fan_damage : public SpellScriptLoader
{
public:
    spell_dk_six_feather_fan_damage() : SpellScriptLoader("spell_dk_six_feather_fan_damage") { }

    class spell_dk_six_feather_fan_damage_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dk_six_feather_fan_damage_SpellScript);
        ...
        void HandleDamage(SpellEffIndex /*effIndex*/)
        {
            ...
            if (target && caster)
            {
                SetHitDamage(urand(30698, 33928));
            }
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_six_feather_fan_damage_SpellScript::HandleDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_dk_six_feather_fan_damage_SpellScript();
    }
};
...
public:
    ...
    public:
        spell_dk_frost_fever_AuraScript() {}


        void CalculateAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& /*canBeRecalculated*/)
        {
            int32 apbonus = int32(GetCaster()->GetTotalAttackPowerValue(BASE_ATTACK) * 0.55f);
            amount = apbonus;
        }
...
public:
    ...
    class spell_dk_heart_strike_SpellScript : public SpellScript
    {
        ...
        void HandleEnergize(SpellEffIndex effIndex)
        {
            ...
            caster->EnergizeBySpell(caster, GetSpellInfo()->Id, GetSpellInfo()->GetEffect(effIndex)->BasePoints * 10, POWER_RUNIC_POWER);

            if (caster->HasAura(221536))
                caster->CastSpell(caster, 210738, true);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_heart_strike_SpellScript::HandleEnergize, EFFECT_2, SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    ...
    class spell_dk_soulgorge_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            caster->GetAnyUnitListInRange(targets, 30.0f);
            ...
            for (auto target : targets)
            {
                ...
                {
                    if (target->HasAura(55078))
                    {
                        Aura* aura = caster->GetAura(55078);
                        if (!aura)
                            continue;

                        int32 bp = round(0.6 * (aura->GetDuration() / 1000));
                        caster->CastCustomSpell(caster, 213003, &bp, NULL, NULL, true);

                        target->RemoveAurasDueToSpell(55078);
                    }
                }
            }
        }
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_soulgorge_SpellScript::HandleDummy, EFFECT_1, SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    ...
    class spell_dk_tombstone_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dk_tombstone_SpellScript);
        ...
        void HandleEnergize(SpellEffIndex /*effIndex*/)
        {
            ...
            {
                if (uint32 stack = aura->GetStackAmount())
                {
                    if (stack > 5)
                        stack = 5;

                    caster->EnergizeBySpell(caster, GetSpellInfo()->Id, (6 * stack) * 10, POWER_RUNIC_POWER);

                    aura->SetStackAmount(aura->GetStackAmount() - stack);
                }
            }
        }
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_tombstone_SpellScript::HandleEnergize, EFFECT_1, SPELL_EFFECT_ENERGIZE);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_dk_tombstone_SpellScript();
    }
};
...
void AddSC_deathknight_spell_scripts()
{
    ...
    new spell_dk_icecap();
    new spell_dk_avalanche();
    new spell_dk_purgatory();
    new spell_dk_shroud_of_purgatory();
    ...
    new spell_dk_six_feather_fan();
    new spell_dk_six_feather_fan_damage();
