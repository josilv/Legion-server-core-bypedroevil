...
class spell_pal_judgement : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_pal_judgement::HandleDummy, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
...
public:
    ...
    class spell_pal_sacred_shield_absorb_AuraScript : public AuraScript
    {
        ...
        void CalculateAmount(AuraEffect const* /*aurEff*/, int32 & amount, bool &)
        {
            ...
            if (GetCaster()->ToPlayer()->GetActiveTalentGroup() == SPEC_PALADIN_HOLY)
                amount = int32(1 + 0.995f * (GetCaster()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY)));
            else if (GetCaster()->ToPlayer()->GetActiveTalentGroup() == SPEC_PALADIN_PROTECTION)
                amount = int32(1 + 1.306f * (GetCaster()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY)));
            else
                amount = int32((1 + 1.306f) * (GetCaster()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY) / 0.7));
        }
...
// Updated 7.0.3
class spell_pal_templar_s_verdict_damage : public SpellScriptLoader
{
public:
    spell_pal_templar_s_verdict_damage() : SpellScriptLoader("spell_pal_templar_s_verdict_damage") { }

    class spell_pal_templar_s_verdict_damage_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_pal_templar_s_verdict_damage_SpellScript);
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_pal_templar_s_verdict_damage_SpellScript::ChangeDamage, EFFECT_0, SPELL_EFFECT_WEAPON_PERCENT_DAMAGE);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_pal_templar_s_verdict_damage_SpellScript();
    }
};
...
public:
    ...
    class spell_pal_seal_of_righteousness_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            GetTarget()->CastCustomSpell(SPELL_PALADIN_SEAL_OF_RIGHTEOUSNESS_DAMAGE, SPELLVALUE_BASE_POINT0, bp, eventInfo.GetProcTarget(), true, NULL, aurEff);
        }
...
public:
    ...
    class spell_pal_seal_of_command_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            GetCaster()->CastSpell(target, SPELL_PALADIN_SEAL_OF_COMMAND, true);
        }
...
public:
    ...
    class spell_pal_exorcism_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(SPELL_PALADIN_EXORCISM);
            if (!spellInfo)
                return;

            GetCaster()->GetSpellHistory()->ResetCooldown(SPELL_PALADIN_EXORCISM, true);
            //GetCaster()->GetSpellHistory()->ResetCharges(spellInfo->ChargeCategoryEntry);
        }

        void Register()
        {
            OnEffectProc += AuraEffectProcFn(spell_pal_exorcism_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_pal_exorcism_AuraScript();
    }
};
    ...
    class spell_pal_eternal_flame_SpellScript : public SpellScript
    {
        ...
        void HandleOnHit()
        {
            if (Player* _player = GetCaster()->ToPlayer())
            {
                if (Unit* unitTarget = GetHitUnit())
                {
                    uint32 holyPower = _player->GetPower(POWER_HOLY_POWER);
                    uint32 heal = int32(_player->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_HOLY) * (1.10f * holyPower));

                    if (unitTarget == _player)
                        AddPct(heal, 50);

                    SetHitHeal(heal);

                    int32 bp = int32(_player->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_HOLY) * 0.10f);
                    _player->CastCustomSpell(_player, SPELL_PALADIN_ETERNAL_FLAME, &bp, NULL, NULL, true);

                    if (Aura* aura = _player->GetAura(SPELL_PALADIN_ETERNAL_FLAME))
                        aura->SetDuration(10000 * holyPower);

                    if (!_player->HasAura(SPELL_PALADIN_DIVINE_PURPOSE_PROC))
                        _player->ModifyPower(POWER_HOLY_POWER, -(int32(holyPower)));
                }
            }
        }
    ...
    class spell_pal_bulwark_of_order_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            if (Player* player = GetTarget()->ToPlayer())
                if (eventInfo.GetDamageInfo()->GetSpellInfo()->Id != 31935)
                    return false;
            return true;
        }
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            if (Player* player = GetCaster()->ToPlayer())
            {
                int32 absorb = CalculatePct(eventInfo.GetDamageInfo()->GetDamage(), 50);
                player->CastCustomSpell(player, 209388, &absorb, NULL, NULL, true);
            }
        }

        void Register() override
        {
            DoCheckProc += AuraCheckProcFn(spell_pal_bulwark_of_order_AuraScript::CheckProc);
            OnEffectProc += AuraEffectProcFn(spell_pal_bulwark_of_order_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
        }
    };
...
public:
    ...
    class spell_pal_light_of_the_titans_AuraScript : public AuraScript
    {
        ...
        void CalculateAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& canBeRecalculated)
        {
            ...
            if (Unit* caster = GetCaster())
                amount = CalculatePct(caster->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_HOLY), 500);
        }

        void Register() override
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pal_light_of_the_titans_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_PERIODIC_HEAL);
        }
    };
...
// 209224 Resolve of the truth
class spell_pal_resolve_of_the_truth : public SpellScriptLoader
{
public:
    spell_pal_resolve_of_the_truth() : SpellScriptLoader("spell_pal_resolve_of_the_truth") { }

    class spell_pal_resolve_of_the_truth_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_pal_resolve_of_the_truth_AuraScript);
        ...
        void CalculateAmount(AuraEffect const* aurEff, int32& amount, bool& canBeRecalculated)
        {
            ...
            if (Unit* caster = GetCaster())
            {
                ...
                maxHealth += CalculatePct(caster->GetMaxHealth(), aurEff->GetSpellInfo()->GetRank());
        ...
        void Register() override
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pal_resolve_of_the_truth_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_MOD_INCREASE_HEALTH_PERCENT);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_pal_resolve_of_the_truth_AuraScript();
    }
};
...
public:
    ...
    class spell_pal_eye_of_tyr_SpellScript : public SpellScript
    {
        ...
        void HandleDamage(SpellEffIndex effIndex)
        {
            ...
            if (Player* player = GetCaster()->ToPlayer())
            {
                uint32 damage = CalculatePct(player->GetTotalAttackPowerValue(BASE_ATTACK), 270);
        ...
        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_pal_eye_of_tyr_SpellScript::HandleDamage, EFFECT_1, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };
...
class spell_pal_tyr_deliverance : public SpellScriptLoader
{
public:
    spell_pal_tyr_deliverance() : SpellScriptLoader("spell_pal_tyr_deliverance") { }

    class spell_pal_tyr_deliverance_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_pal_tyr_deliverance_AuraScript);
        ...
        void HandlePeriodic(AuraEffect const* aurEff)
        {
            ...
            {
                if (Unit* target = ObjectAccessor::GetUnit(*GetCaster(), *targetGUID))
                    GetCaster()->CastSpell(target, 200654, true);
            }
            else
            {
                std::list<Unit*> targets;
                GetCaster()->GetAnyUnitListInRange(targets, 15.0f);
                ...
                for (auto target : targets)
                {
                    ...
                    {
                        GetCaster()->GetAura(GetSpellInfo()->Id)->SetCastExtraParam("targetGUID", target->GetGUID());
                        GetCaster()->CastSpell(target, 200654, true);
                    }
                }
            }
        ...
        void Register()
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_pal_tyr_deliverance_AuraScript::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_pal_tyr_deliverance_AuraScript();
    }
};
...
void AddSC_paladin_spell_scripts()
{
    ...
    new spell_pal_templar_s_verdict_damage();
    ...
    new spell_pal_resolve_of_the_truth();
    new spell_pal_eye_of_tyr();
    new spell_pal_tyr_deliverance();
}
