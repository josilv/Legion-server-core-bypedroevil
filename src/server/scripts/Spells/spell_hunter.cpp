                ...
                if (Unit* caster = GetCaster())
                    if (Unit* target = GetHitUnit())
                        if (caster == GetOriginalCaster())
                            target->CastSpell(caster, SPELL_HUNTER_GLAIVE_TOSS_LEFT, true, NULL, 0, caster->GetGUID());
            }
            else
            {
                if (Unit* caster = GetCaster())
                    if (Unit* target = GetHitUnit())
                        if (caster == GetOriginalCaster())
                            target->CastSpell(caster, SPELL_HUNTER_GLAIVE_TOSS_RIGHT, true, NULL, 0, caster->GetGUID());
            }
        }
...
// Bond Spirit - 118694
class spell_hun_bond_spirit : public SpellScriptLoader
{
public:
    spell_hun_bond_spirit() : SpellScriptLoader("spell_hun_bond_spirit") { }

    class spell_hun_bond_spirit_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_hun_bond_spirit_AuraScript);
        ...
        void HandleEffectPeriodicUpdate(AuraEffect* aurEff)
        {
            ...
            if (GetCaster()->GetHealth() < GetCaster()->GetMaxHealth() || GetCaster()->GetOwner()->GetHealth() < GetCaster()->GetOwner()->GetMaxHealth())
                GetCaster()->CastSpell(GetCaster()->GetOwner(), SPELL_HUNTER_BOND_SPIRIT_HEAL, true, 0, 0, GetCaster()->GetGUID());
        }

        void Register()
        {
            OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(spell_hun_bond_spirit_AuraScript::HandleEffectPeriodicUpdate, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_hun_bond_spirit_AuraScript();
    }
};
...
public:
    ...
    class spell_hun_steady_focus_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo &procInfo)
        {
            ...
            uint32 spellId = procInfo.GetDamageInfo()->GetSpellInfo()->Id;

            if (!spellId)
                return;

            if (spellId == SPELL_HUNTER_FOCUSING_SHOT)
                spellId = SPELL_HUNTER_COBRA_SHOT;

            if (spellId == SPELL_HUNTER_FOCUSING_SHOT_AIM)
                spellId = SPELL_HUNTER_STEADY_SHOT;

            switch (spellId)
            {
            case SPELL_HUNTER_STEADY_SHOT:
            case SPELL_HUNTER_COBRA_SHOT:
            {
                aura->SetCharges(aura->GetCharges() + 1);
                if (aura->GetCharges() >= 2)
                {
                    GetTarget()->CastSpell(GetTarget(), SPELL_HUNTER_STEADY_FOCUS_TRIGGERED, true);
                    aura->SetCharges(0);
                    break;
                }
            }
        ...
        void Register() override
        {
            DoCheckProc += AuraCheckProcFn(spell_hun_steady_focus_AuraScript::CheckProc);
            OnEffectProc += AuraEffectProcFn(spell_hun_steady_focus_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
        }
    };
...
// 155228 - Lone Wolf
class spell_hun_lone_wolf_aura : public SpellScriptLoader
{
public:
    spell_hun_lone_wolf_aura() : SpellScriptLoader("spell_hun_lone_wolf_aura") { }

    class spell_hun_lone_wolf_aura_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_hun_lone_wolf_aura_AuraScript);
        ...
        void Register()
        {
            AfterEffectApply += AuraEffectApplyFn(spell_hun_lone_wolf_aura_AuraScript::OnApply, EFFECT_0, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_hun_lone_wolf_aura_AuraScript();
    }
};
...
public:
    ...
    class spell_hun_aspect_of_the_turtle_AuraScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const*  /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
            {
                caster->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED);
                caster->AttackStop();
            }
        }
        ...
        {
            if (Unit* caster = GetCaster())
                caster->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED);
        }
...
public:
    ...
    class spell_hun_aimed_shot_SpellScript : public SpellScript
    {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_hun_aimed_shot_SpellScript::HandleDamage, EFFECT_1, SPELL_EFFECT_WEAPON_PERCENT_DAMAGE);
        }
    };
...
public:
    ...
    class spell_hun_explosive_shot_aura_AuraScript : public AuraScript
    {
        ...
        void Register()
        {
            AfterEffectApply += AuraEffectApplyFn(spell_hun_explosive_shot_aura_AuraScript::OnApply, EFFECT_0, SPELL_AURA_OVERRIDE_ACTIONBAR_SPELLS, AURA_EFFECT_HANDLE_REAL);
            AfterEffectApply += AuraEffectApplyFn(spell_hun_explosive_shot_aura_AuraScript::OnApply1, EFFECT_1, SPELL_AURA_AREA_TRIGGER, AURA_EFFECT_HANDLE_REAL);
        }
    };
...
void AddSC_hunter_spell_scripts()
{
    ...
    new spell_hun_bond_spirit();
    new spell_hun_steady_focus();
    new spell_hun_blink_strike();
    new spell_hun_lone_wolf_aura();
