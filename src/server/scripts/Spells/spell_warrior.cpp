...
enum WarriorSpells
{
    ...
    SPELL_WARR_EXECUTE                              = 163201,
    SPELL_WARRIOR_ART_CONCORDANCE_OF_LEGIONFALL     = 239042,
    SPELL_WARRIOR_ART_CONCORDANCE_OF_LEGIONFALL_AURA = 242583,
    SPELL_WARR_ART_EXECUTIONES_PRECISION            = 238147,
...
public:
    ...
    class spell_warr_bloodthirst_heal_SpellScript : public SpellScript
    {
        ...
        {
            uint32 healPct = GetHitHeal();

            if (Unit* caster = GetCaster())
                if (caster->HasAura(202224)) // Furious Charge
                    healPct += CalculatePct(healPct, 300);
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_warr_bloodthirst_heal_SpellScript::HandleHealPct, EFFECT_0, SPELL_EFFECT_HEAL_PCT);
        }
    };
...
public:
    ...
    class spell_warr_second_wind_trigger_AuraScript : public AuraScript
    {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            return ValidateSpellInfo({ SPELL_WARRIOR_SECOND_WIND_DAMAGED });
        }

        void OnPeriodic(AuraEffect const* /*aurEff*/)
        {
            if (GetCaster()->HasAura(SPELL_WARRIOR_SECOND_WIND_DAMAGED))
...
/// 7.2.5
class at_spell_warr_into_the_fray : public AreaTriggerEntityScript
{
public:
    at_spell_warr_into_the_fray() : AreaTriggerEntityScript("at_spell_warr_into_the_fray") { }

    struct at_spell_warr_into_the_fray_AI : AreaTriggerAI
    {
        at_spell_warr_into_the_fray_AI(AreaTrigger* areatrigger) : AreaTriggerAI(areatrigger) { }

        enum Spells
        {
            SPELL_WARRIOR_INTO_THE_FRAY_BUFF = 202602,
        };

        void OnUnitEnter(Unit* unit) override
        {
            if (Unit* caster = at->GetCaster())
                if (!caster->IsFriendlyTo(unit) && caster != unit)
                    caster->CastSpell(caster, SPELL_WARRIOR_INTO_THE_FRAY_BUFF, true);
        }
        ...
        void OnUnitExit(Unit* unit) override
        {
            if (Unit* caster = at->GetCaster())
                if (!caster->IsFriendlyTo(unit) && caster != unit)
                    caster->RemoveAuraFromStack(SPELL_WARRIOR_INTO_THE_FRAY_BUFF);
        }
    ...
    {
        return new at_spell_warr_into_the_fray_AI(areatrigger);
    }
};
    ...
    class spell_warr_anger_management_AuraScript : public AuraScript
    {
        ...
        void HandleEffectProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            if (Player* player = GetCaster()->ToPlayer())
            {
                ...
                std::vector<SpellPowerCost> costs = spellInfo->CalcPowerCost(GetTarget(), SpellSchoolMask(eventInfo.GetDamageInfo()->GetSchoolMask()));
                ...
                if (m != costs.end())
                    if (m->Amount >= 10)
                    {
                        ...
                        {
                            seconds = round(m->Amount / 200);
                            player->GetSpellHistory()->ModifyCooldown(SPELL_WARRIOR_BLADESTORM, -(seconds)* IN_MILLISECONDS);
                        }

                        if (player->GetUInt32Value(PLAYER_FIELD_CURRENT_SPEC_ID) == SPEC_WARRIOR_PROTECTION)
                        {
                            seconds = round(m->Amount / 100);

                            player->GetSpellHistory()->ModifyCooldown(SPELL_WARRIOR_LAST_STAND, -(seconds)* IN_MILLISECONDS);
                            player->GetSpellHistory()->ModifyCooldown(SPELL_WARRIOR_SHIELD_WALL, -(seconds)* IN_MILLISECONDS);
                            player->GetSpellHistory()->ModifyCooldown(SPELL_WARRIOR_DEMORALIZING_SHOUT, -(seconds)* IN_MILLISECONDS);
                        }

                        player->GetSpellHistory()->ModifyCooldown(SPELL_WARRIOR_BATTLE_CRY, -(seconds)* IN_MILLISECONDS);
                    }
            }
        }
...
// ID - 199854 Tactician Cooldown Handler
class spell_warr_tactician_cd : public SpellScriptLoader
{
public:
    spell_warr_tactician_cd() : SpellScriptLoader("spell_warr_tactician_cd") { }

    class spell_warr_tactician_cd_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_warr_tactician_cd_AuraScript);
        ...
        void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            {
                SpellInfo const* colossusSmash = sSpellMgr->GetSpellInfo(SPELL_WARRIOR_COLOSSUS_SMASH);
                player->GetSpellHistory()->ResetCooldown(SPELL_WARRIOR_COLOSSUS_SMASH, true);
                player->GetSpellHistory()->ResetCharges(colossusSmash->ChargeCategoryId);
                SpellInfo const* mortalStrike = sSpellMgr->GetSpellInfo(SPELL_WARRIOR_MORTAL_STRIKE);
                player->GetSpellHistory()->ResetCooldown(SPELL_WARRIOR_MORTAL_STRIKE, true);
                player->GetSpellHistory()->ResetCharges(mortalStrike->ChargeCategoryId);
            }
        }

        void Register() override
        {
            AfterEffectApply += AuraEffectRemoveFn(spell_warr_tactician_cd_AuraScript::HandleApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_warr_tactician_cd_AuraScript();
    }
};
...
public:
    ...
    class spell_warr_deep_wounds_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            if (eventInfo.GetSpellInfo()->Id == 20243 || eventInfo.GetSpellInfo()->Id == 6572)
                return true;
        ...
        void CalculateAmount(AuraEffect const* aurEff, int32& amount, bool& canBeRecalculated)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                amount = CalculatePct(caster->GetTotalAttackPowerValue(BASE_ATTACK), 105);
            }
        }

        void Register() override
        {
            DoCheckProc += AuraCheckProcFn(spell_warr_deep_wounds_AuraScript::CheckProc);
...
public:
    ...
    class spell_warr_berserker_rage_SpellScript : public SpellScript
    {
        ...
        enum Spells
        {
            ...
            SPELL_WARRIOR_ENRAGE_BUFF = 184362,
        };
        ...
        void HandleCast()
        {
            if (Unit* caster = GetCaster())
            {
                if (caster->HasAura(SPELL_WARRIOR_OUTBURST))
                    caster->CastSpell(caster, SPELL_WARRIOR_ENRAGE_BUFF, true);
            }
        }

        void Register() override
        {
            OnCast += SpellCastFn(spell_warr_berserker_rage_SpellScript::HandleCast);
        }
    };
...
public:
    ...
    class spell_warr_inspiring_presence_AuraScript : public AuraScript
    {
        ...
        void PeriodicUpdate(AuraEffect const* aurEff)
        {
            PreventDefaultAction();
            ...
            if (Player* player = GetCaster()->ToPlayer())
            {
                ...
                if (Group* group = player->GetGroup())
                {
                    for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
                    {
                        ...
                        bp0 += member->GetDamageDoneInPastSecs(1);
                    }

                    heal = CalculatePct(bp0, 3);

                    for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
                    {
                        Player* member = itr->GetSource();
                        player->CastCustomSpell(member, 222944, &heal, NULL, NULL, true);
                    }
                }
                else
                {
                    bp0 += player->GetDamageDoneInPastSecs(1);
                    player->CastCustomSpell(player, 222944, &bp0, NULL, NULL, true);
                }
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_warr_inspiring_presence_AuraScript::PeriodicUpdate, EFFECT_1, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };
...
public:
    ...
    class spell_warr_revenge_SpellScript : public SpellScript
    {
        ...
        void HandleDamage(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
            {
                if (caster->HasAura(202572))
                    caster->CastSpell(caster, 202574, true);
            }
            ...
            if (Unit* caster = GetCaster())
            {
                if (caster->HasAura(202560)) // Best Served Cold
                    return;
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_warr_revenge_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_CONE_ENEMY_104);
            OnEffectHitTarget += SpellEffectFn(spell_warr_revenge_SpellScript::HandleDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };
...
// 236279 - Devastator
class spell_warr_devastator : public SpellScriptLoader
{
public:
    spell_warr_devastator() : SpellScriptLoader("spell_warr_devastator") { }

    class spell_warr_devastator_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_warr_devastator_SpellScript);
        ...
        {
            if (Unit* caster = GetCaster())
                if (roll_chance_i(GetSpellInfo()->GetEffect(effIndex)->BasePoints))
                    caster->GetSpellHistory()->ResetCooldown(SPELL_WARRIOR_SHIELD_SLAM, true);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_warr_devastator_SpellScript::HandleDummy, EFFECT_1, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_warr_devastator_SpellScript();
    }
};

// 206313 - Frenzy
class spell_warr_frenzy_proc : public SpellScriptLoader
{
public:
    spell_warr_frenzy_proc() : SpellScriptLoader("spell_warr_frenzy_proc") { }

    class spell_warr_frenzy_proc_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_warr_frenzy_proc_AuraScript);

        bool CheckProc(ProcEventInfo& eventInfo)
        {
            if (eventInfo.GetDamageInfo()->GetSpellInfo()->Id == 100130)
                return true;
        ...
        void HandleProc(ProcEventInfo& eventInfo)
        {
            ...
            if (Unit* caster = GetCaster())
                caster->CastSpell(caster, 202539, true);
        }

        void Register() override
        {
            DoCheckProc += AuraCheckProcFn(spell_warr_frenzy_proc_AuraScript::CheckProc);
            OnProc += AuraProcFn(spell_warr_frenzy_proc_AuraScript::HandleProc);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_warr_frenzy_proc_AuraScript();
    }
};
        ...
        void HandleProc(ProcEventInfo& eventInfo)
        {
            if (Unit* caster = GetCaster())
                if (Unit* target = eventInfo.GetActionTarget())
                    caster->CastSpell(target, 113344, true);
        }

        void Register() override
        {
            OnProc += AuraProcFn(spell_warr_bloodbath_AuraScript::HandleProc);
        }
    };
...
//ID - 238147 Executioner's Precision
class spell_warr_executioner_precision : public AuraScript
{
    PrepareAuraScript(spell_warr_executioner_precision);

    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({ SPELL_WARR_ART_EXECUTIONES_PRECISION });
    }
    ...
    bool OnCheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (eventInfo.GetSpellInfo())
            if (eventInfo.GetSpellInfo()->Id != SPELL_WARR_EXECUTE)
                return false;
    ...
    void Register() override
    {
        DoCheckProc += AuraCheckProcFn(spell_warr_executioner_precision::OnCheckProc);
    }
};


//ID - 239042 Concordance of the Legionfall
class spell_warr_concordance_of_legionfall : public AuraScript
{
    PrepareAuraScript(spell_warr_concordance_of_legionfall);

    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({ SPELL_WARRIOR_ART_CONCORDANCE_OF_LEGIONFALL, SPELL_WARRIOR_ART_CONCORDANCE_OF_LEGIONFALL_AURA });
    }
    ...
    void OnProcAura(ProcEventInfo& eventInfo)
    {
        PreventDefaultAction();
        Unit* caster = GetCaster();
        if (!caster)
        ...
        int32 amount = 0;
        if (eventInfo.GetSpellInfo())
            if (Aura* aura = caster->GetAura(SPELL_WARRIOR_ART_CONCORDANCE_OF_LEGIONFALL))
                amount = (aura->GetEffect(EFFECT_0)->GetAmount());
        
        if(amount>0)
            caster->CastCustomSpell(SPELL_WARRIOR_ART_CONCORDANCE_OF_LEGIONFALL_AURA, SPELLVALUE_BASE_POINT0, amount, caster, TRIGGERED_FULL_MASK);
        return;
    }

    void Register() override
    {
        DoCheckProc += AuraCheckProcFn(spell_warr_concordance_of_legionfall::OnCheckProc);
        OnProc += AuraProcFn(spell_warr_concordance_of_legionfall::OnProcAura);
    }
};
...
void AddSC_warrior_spell_scripts()
{
	...
	RegisterAuraScript(spell_warr_executioner_precision);
    RegisterAuraScript(spell_warr_concordance_of_legionfall);
