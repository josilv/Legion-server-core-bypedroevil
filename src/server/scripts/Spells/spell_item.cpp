...
// 213712
class spell_item_careful_swing : public SpellScriptLoader
{
public:
    spell_item_careful_swing() : SpellScriptLoader("spell_item_careful_swing") { }

    class spell_item_careful_swing_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_item_careful_swing_SpellScript);

        SpellCastResult CheckCast()
        {
            if (Unit* target = GetHitUnit())
                if (target->GetTypeId() != TYPEID_PLAYER || !target->ToCreature()->isWorldBoss() || !target->ToCreature()->IsDungeonBoss())
                    return SPELL_CAST_OK;

            return SPELL_FAILED_BAD_TARGET_FILTER;
        }

        void Register() override
        {
            OnCheckCast += SpellCheckCastFn(spell_item_careful_swing_SpellScript::CheckCast);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_item_careful_swing_SpellScript();
    }
};
